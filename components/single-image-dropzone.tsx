...
const SingleImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const imageUrl = React.useMemo(() => {
      if (!isBrowser) return null; // Ensure SSR safety
      if (typeof value === 'string') {
        return value; // Use external URL if provided
      }
      if (value instanceof File) {
        return URL.createObjectURL(value); // Generate a URL for the file
      }
      return null;
    }, [value, isBrowser]);
    ...
    React.useEffect(() => {
      return () => {
        if (imageUrl && typeof value !== 'string') {
